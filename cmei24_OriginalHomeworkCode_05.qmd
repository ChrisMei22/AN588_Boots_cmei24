...
for (i in 1:1000){
  sample_rows <- KC_data[sample(nrow(KC_data), nrow(KC_data), replace = TRUE),]
...
When it comes to SE, the SE from the bootstrapped method is lower than the original one (entire dataset).
...
Compared to the bootstrapped CI, both values are very similar with the 97.5 quantile being higher in the original dataset and the 2.5 quantile being higher in the boostrapped dataset.
...
lm_bootstrap <- function(d, linear_model, m, conf.level = 0.95, n = 1000){
  ...
  set.seed(1)
  for (i in 1:n){
    sample_rows <- d[sample(nrow(d), nrow(d), replace = TRUE),]
